...
public class TestDataRunner implements ApplicationRunner {
    ...
    // 스프링이 기동될 때 코드들이 실행 됨
    @Override
    public void run(ApplicationArguments args) throws Exception {
        // 테스트 User 생성
        User testUser1 = new User("슈가", passwordEncoder.encode("123"), "sugar@sparta.com", UserRoleEnum.USER);
        User testUser2 = new User("제이홉", passwordEncoder.encode("123"), "j@sparta.com", UserRoleEnum.USER);
        User testAdminUser1 = new User("아미", passwordEncoder.encode("123"), "army@sparta.com", UserRoleEnum.ADMIN);

        // save하면 생성되는 1차 캐시의 데이터를 기존 객체에 덮어 씌움
